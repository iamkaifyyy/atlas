    ...
    <div className="cohere-card-console p-5 space-y-4 font-sans">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2.5">
          <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-console-elevated text-coral font-bold text-xs border border-console-border font-mono">
            1
          </span>
      ...
      <div className="grid grid-cols-2 gap-3 font-mono">
        <button
          ...
          className={`flex items-center justify-center gap-2 p-3 rounded-lg border transition text-xs font-semibold ${
            value.type === 'PRICE_BELOW'
              ? 'bg-console-elevated border-coral/60 text-coral shadow-sm'
              : 'bg-transparent border-console-border text-muted hover:text-white hover:border-muted'
          }`}
        >
      ...
      <div>
        <label className="block text-xs text-muted mb-1.5 font-mono uppercase tracking-[0.28px]">
          Target Price
        </label>
        <div className="relative rounded-lg">
          <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-muted font-mono text-xs">
            $
          </span>
          <input
            type="number"
            ...
            placeholder="3050"
            className="block w-full pl-8 pr-16 py-2.5 bg-console-elevated border border-console-border rounded-lg text-white font-mono text-sm focus:outline-none focus:border-coral transition"
          />
